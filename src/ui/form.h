...
public:
	...
	{
	public:
		virtual void process(UI::Frame &ctx, int ch) = 0;
	...
	Form(std::string title, FieldList &&fields, action_t action);
...
{
public:
	Input(std::string caption, std::string value);
	virtual void process(UI::Frame &ctx, int ch) override;
